...
<jsp:include page="common/header.jsp"><jsp:param name="pageTitle" value="Add Product"/></jsp:include>
...
<div class="container-fluid py-4">
 <div class="row justify-content-center"><div class="col-lg-8">
  <div class="d-flex align-items-center mb-4">
   ...
   <div><h1 class="page-title mb-0">Add New Product</h1><p class="text-muted mb-0">Enter product details</p></div>
  </div>
  ...
  <div class="card dashboard-card"><div class="card-body p-4">
   <form action="${pageContext.request.contextPath}/products/add" method="post" id="addProductForm" onsubmit="return validateProductForm()">
    <div class="mb-3"><label for="name" class="form-label fw-semibold">Product Name <span class="text-danger">*</span></label>
     <input type="text" class="form-control" id="name" name="name" value="${product.name}" placeholder="Enter product name" required></div>
    <div class="mb-3"><label for="description" class="form-label fw-semibold">Description</label>
     <textarea class="form-control" id="description" name="description" rows="3" placeholder="Enter description">${product.description}</textarea></div>
    <div class="row g-3 mb-3">
     ...
     <div class="col-md-4"><label for="gstPercentage" class="form-label fw-semibold">GST % <span class="text-danger">*</span></label>
      <div class="input-group"><input type="number" class="form-control" id="gstPercentage" name="gstPercentage" value="${product.gstPercentage != null ? product.gstPercentage : '18.00'}" step="0.01" min="0" max="100" required><span class="input-group-text">%</span></div></div>
    ...
    <div class="d-flex justify-content-end gap-2">
     ...
     <button type="submit" class="btn btn-primary"><i class="bi bi-check-circle me-1"></i>Add Product</button>
    </div>
   </form>
  </div></div>
 </div></div>
...
function validateProductForm(){
 ...
 if(isNaN(p)||p<=0){alert('Price must be greater than zero');return false;}
 if(isNaN(g)||g<0||g>100){alert('GST must be between 0 and 100');return false;}
